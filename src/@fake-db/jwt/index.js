...
mock.onPost('/auth/register').reply(request => {
  ...
  const errors = {
    password: !password ? ['Introduza a senha'] : null,
    email: (() => {
      if (!email)
        return ['Introduza o seu e-mail.']
      if (isEmailAlreadyInUse)
        return ['O e-mail introduzido ja esta em uso.']
      
      return null
    })(),
    username: (() => {
      if (!username)
        return ['Introduza o seu username.']
      if (isUsernameAlreadyInUse)
        return ['O username introduzido ja esta em uso.']
