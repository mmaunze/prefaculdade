  ...
  >
    ...
    <VCardText>
      <VList class="card-list">
        <VListItem
          v-for="provincia in salesByCountries"
          :key="provincia.stats"
        >
          <template #prepend>
            <VAvatar
          ...
          <template #append>
            <div
              :class="`d-flex align-center font-weight-medium ${
                provincia.profitLoss > 0 ? 'text-success' : 'text-error'
              }`"
            >
              <VIcon
                :icon="
                  provincia.profitLoss > 0
                    ? 'tabler-chevron-up'
                    : 'tabler-chevron-down'
                "
                size="18"
                class="me-1"
              />
              <span>{{ Math.abs(provincia.profitLoss) }}%</span>
            </div>
          </template>
        </VListItem>
      </VList>
